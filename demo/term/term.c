...
static void xterm_sequence (s_window_t *window, int fd, int op)
{
	...
	switch (op) {
		case 2:
			buf_ = (unsigned char *) s_malloc(strlen((char *) buf) + strlen("XFasterminal - "));
			sprintf((char *) buf_, "XFasterminal - %s", buf);
...
static void write_char (s_window_t *window, unsigned chr)
{
	...
	if (s_surface_create(&s, FONTW, FONTH, window->surface->bitsperpixel)) {
		goto out;
	}
...
int main (int argc, char *argv[])
{
	...
	s_window_set_title(window, "XFasterminal");
