...
################################
	...
	if test "$BUILD_GTK" = "1" ; then
	    $WGET ftp://ftp.gtk.org/pub/gtk/2.12/$GTK.tar.bz2 || { echo "ERROR DOWNLOADING GTK"; exit; }
	fi
...
#######################
	...
	../configure --prefix=$PREFIX --with-gdktarget=directfb || { echo "ERROR CONFIGURING GTK"; exit; }
	## Build the source.
