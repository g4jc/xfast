...
static int s_thread_pspdev_sem_create (s_thread_sem_t *sem, int initial)
{
	...
	sem->sem_id = sceKernelCreateSema("XFast Sema", 0, initial, 255, NULL);
	if (sem->sem_id < 0) {
		goto err;
	}
	return 0;
...
static int s_thread_pspdev_thread_create (s_thread_t *tid, s_thread_arg_t *targs)
{
	...
	tid->tid = sceKernelCreateThread("XFast Thread", ThreadEntry, priority, 0x8000, PSP_THREAD_ATTR_USER, NULL);
	if (tid->tid < 0) {
		goto err;
	}
