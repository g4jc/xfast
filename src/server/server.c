...
int s_server_cfg (s_server_conf_t *config)
{
	...
	s_config_init(&cfg);
	if (s_config_parse(cfg, CONFIG_PATH_INSTALL CONFIG_PATH_CONFIGS "/xfast.conf")) {
		debugf(DSER, "Configuration file parsing failed (%s)", CONFIG_PATH_INSTALL CONFIG_PATH_CONFIGS "/xfast.conf");
		s_config_uninit(cfg);
		return -1;
	}
