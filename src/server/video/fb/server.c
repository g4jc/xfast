...
void fb_server_surface_init (s_video_helper_mode_info_t *gmode)
{
	...
	if (server->window->surface->linear_mem_size){
		server->window->surface->linear_mem_base = (unsigned int) mmap(NULL, server->window->surface->linear_mem_size,
...
{
	munmap((void *) server->window->surface->linear_mem_base, (size_t)server->window->surface->linear_mem_size);
}
