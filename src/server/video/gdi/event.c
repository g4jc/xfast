...
void * s_video_gdi_create_window (void *arg)
{
	...
	HDC mainwindow;
	WNDCLASSEX wndclass;
	char *szMainWndClass = "XFastWindowingSystem";
	s_video_gdi_data_t *priv = (s_video_gdi_data_t *) server->driver->driver_data;
	...
	if (mainwindow == NULL) {
                debugf(DSER | DFAT, "What the ... is this?");
		return NULL;
	}
	priv->bpp_windows = GetDeviceCaps(mainwindow, BITSPIXEL);
	if (priv->bpp_windows == 24) {
                debugf(DSER | DFAT, "24 bits per pixel modes are not supported by XFast Windowing System. "
